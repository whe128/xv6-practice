...
freevm(pde_t *pgdir)
{
  ...
  deallocuvm(pgdir, KERNBASE, USERBASE);
...
int user_mem_writable(char *addr, int len)
{
  int npages;
  ...
  npages = (len + PGSIZE - 1) / PGSIZE;

  // not currently a part of the address space
  for (int i = 0; i < npages; i++)
  {
    pte_t *pte = walkpgdir(myproc()->pgdir, start_addr + i * PGSIZE, 1);
    // no page table entry
    if (pte == 0)
    {
      return 0;
